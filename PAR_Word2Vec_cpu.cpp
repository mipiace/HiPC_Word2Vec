...
void Train_SGNS() {
    ...
    #pragma omp parallel num_threads(num_threads)
    {
        ...
        // temporary memory
        real * neu1e = (real *) _mm_malloc(hidden_size * sizeof(real), 64);
        real * inputM = (real *) _mm_malloc((batch_size*hidden_size) * sizeof(real), 64);
        real * outputM = (real *) _mm_malloc((window*2*negative*hidden_size) * sizeof(real), 64);
        real * outputMd = (real *) _mm_malloc((window*2*negative*hidden_size) * sizeof(real), 64);
        real * gradM = (real *) _mm_malloc((window*2*negative*batch_size) * sizeof(real), 64);
        ...
        while (1) {
            ...
            if ((disk && feof(fin)) || (word_count > local_train_words)) {
                ...
                //                 fwrite(&Wih[a * hidden_size + b], sizeof(real), 1, foutput);
                //         else
                //             for (int b = 0; b < hidden_size; b++)
                ...
                if (sentence_length > 0) {
                    ...
                    for (int n_b = 0; n_b < num_batch; n_b++) {
                        ...
                        for (int i = 0; i < input_size; i++) {
                            memcpy(inputM + i * hidden_size, Wih + sen[min_word_pos + i] * hidden_size, hidden_size * sizeof(real));
                        }

                        int output_size = num_neg_samples;
                        for (int i = 0; i < output_size; i++) {
                            memcpy(outputM + i * hidden_size, Woh + shared_negative[i] * hidden_size, hidden_size * sizeof(real));
                        }
...
//                                 if (f > MAX_EXP)
//                                     // g = (-1)*alpha; // -alpha
//                                     g = (label - 1) * alpha; // -alpha
//                                 else if (f < -MAX_EXP)
//                                     // g = 0;
//                                     g = label * alpha; // 0
//                                 else
//                                     g = (label - expTable[(int) ((f + MAX_EXP) * EXP_RESOLUTION)]) * alpha;
//                                     // if ((int) ((f + MAX_EXP) * EXP_RESOLUTION) >= (EXP_TABLE_SIZE + 1)) {
//                                     //     printf("(int) ((f + MAX_EXP) * EXP_RESOLUTION) %d\n",(int) ((f + MAX_EXP) * EXP_RESOLUTION));
//                                     //     printf("expTable[] %f\n",expTable[(int) ((f + MAX_EXP) * EXP_RESOLUTION)]);
//                                     // } // occasionally happens
//                                     // expTable = (real *) _mm_malloc((EXP_TABLE_SIZE + 1) * sizeof(real), 64);
//                                 gradM[i * input_size + j] = g;
//                             }
//                         }
                    
// #else
                        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, output_size, input_size, hidden_size, 1.0f, outputM,
                                hidden_size, inputM, hidden_size, 0.0f, gradM, input_size);

                        for (int i = 0; i < output_size; i++) {
                            int offset = i * input_size;
                            #pragma simd
                            for (int j = 0; j < input_size; j++) {
                                real f = gradM[offset + j];
                                int label = 0;
                                if (f > MAX_EXP)
                                    f = (label - 1) * alpha;
                                else if (f < -MAX_EXP)
...
//                                     f += gradM[i * input_size + k] * inputM[k * hidden_size + j];
//                                 }
...
// #else
                        ...
                        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, output_size, hidden_size, input_size, 1.0f, gradM,
...
//                                     f += gradM[k * input_size + i] * outputM[k * hidden_size + j];
//                                 }
//                                 inputM[i * hidden_size + j] = f;
//                             }
//                         }
// #else
                        cblas_sgemm(CblasRowMajor, CblasTrans, CblasNoTrans, input_size, hidden_size, output_size, 1.0f, gradM,
...
// #endif
                    ...
                    } // end of processing all batches

                } // end of if sentence_length > 0
