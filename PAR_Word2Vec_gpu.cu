...
int *vocab_hash = NULL;
int *table = NULL;
float *Wih = NULL, *Woh = NULL, *expTable = NULL;
...
void InitNet() {
    // Wih = (float *) _mm_malloc(vocab_size * hidden_size * sizeof(float), 64);
    // Woh = (float *) _mm_malloc(vocab_size * hidden_size * sizeof(float), 64);
    Wih = (float *)malloc(vocab_size * hidden_size * sizeof(float));
    Woh = (float *)malloc(vocab_size * hidden_size * sizeof(float));
    ...
    for (int i = 0; i < vocab_size; i++) {
        for (int j = 0; j < hidden_size; j++) {
            Wih[i * hidden_size + j] = 0.f;
            Woh[i * hidden_size + j] = 0.f;
        }
    }
    ...
    checkCUDAerr(cudaMalloc((void **)&d_Wih, vocab_size * hidden_size * sizeof(float)));
    checkCUDAerr(cudaMalloc((void **)&d_Woh, vocab_size * hidden_size * sizeof(float)));
    checkCUDAerr(cudaMemcpy(d_Wih, Wih, (vocab_size * hidden_size) * sizeof(int), cudaMemcpyHostToDevice));
    checkCUDAerr(cudaMemcpy(d_Woh, Woh, (vocab_size * hidden_size) * sizeof(int), cudaMemcpyHostToDevice));
...
__global__ void par_sgns_kernel(int num_sentence_blk, int window, int hidden_size, int negative, float alpha, int table_size, int vocab_size, int batch_size, int total_num_sent, const int* __restrict__ d_table, const float* __restrict__ d_expTable, const int* __restrict__ d_corpus, const int* __restrict__ d_sen_len_ptr, float *d_Wih, float *d_Woh, float *inputM, float *inputMd, float *outputM, float *outputMd, float *gradM, int *d_rand_window_batch, int *d_shared_negative)
{
    ...
    if (s_sent_idx_pb < total_num_sent) {
        ...
        for (int sid = s_sent_idx_pb; sid < e_sent_idx_pb; sid++) {
            // const int sent_idx_s = d_sen_len_ptr[d_sentence_idx[start_bin+sid]];
            // const int sent_idx_e = d_sen_len_ptr[d_sentence_idx[start_bin+sid] + 1];
            ...
            if (sent_length > 0) {
                ...
                for (int sentence_position = sent_idx_s; sentence_position < sent_idx_e; sentence_position++) {
                    ...
                    for (int a = b; a < window * 2 + 1 - b; a++) if (a != window) {
                        ...
                        if (threadIdx.x / 32 == 0) {
                            float f2, f3, f4;
                            f = d_Wih[threadIdx.x + l1] * d_Woh[threadIdx.x + l2];
                            f2 = d_Wih[threadIdx.x + l1 + 32] * d_Woh[threadIdx.x + l2 + 32];
                            f3 = d_Wih[threadIdx.x + l1 + 64] * d_Woh[threadIdx.x + l2 + 64];
                            f4 = d_Wih[threadIdx.x + l1 + 96]  * d_Woh[threadIdx.x + l2 + 96];
                            f += f2 + f3 + f4;
                            f += __shfl_down(f, 16);
                            f += __shfl_down(f, 8);
                            f += __shfl_down(f, 4);
                        ...
                        __syncthreads();
                        neu1e += s_vector[0] * d_Woh[threadIdx.x + l2];
                        // d_Woh[threadIdx.x + l2] += s_vector[0] * d_Wih[threadIdx.x + l1];
                        // d_Wih[threadIdx.x + l1] += neu1e;
                        atomicAdd(&d_Woh[threadIdx.x + l2], s_vector[0] * d_Wih[threadIdx.x + l1]);
                        atomicAdd(&d_Wih[threadIdx.x + l1], neu1e);
                    }
                }
                // }
                __syncthreads();
                ...
                for (int batch_id = 0; batch_id < num_batch; batch_id++) {
                    ...
                    for (int j = threadIdx.x; j < output_size; j += blockDim.x) {
                        // seed = abs((int) clock()*100000000) + threadIdx.x + sid;
                        seed = abs((int) clock()*100000000) + threadIdx.x;
                        // seed = threadIdx.x + sid; // not working
                        curandState s;
                        curand_init(seed, 0, 0, &s);
                    ...
                    for (int sen_pos = batch_min_idx; sen_pos < batch_max_idx; sen_pos++) {       // 16
                        ...
                        inputM[blockIdx.x*batch_size*hidden_size + d1 + threadIdx.x] = d_Wih[s1 + threadIdx.x];    
                    }
                    for (int ng_pos = 0; ng_pos < output_size; ng_pos++) {     // 40
                        ...
                        outputM[blockIdx.x*2*window*negative*hidden_size + d2 + threadIdx.x] = d_Woh[s2 + threadIdx.x];
                    }
                    __syncthreads();
                    ...
                    for (int a = threadIdx.x; a < output_size*batch_size; a += blockDim.x) {
                        gradM[blockIdx.x*2*window*negative*batch_size + a] = 0;
                    }
                    ...
                    for (int rb = 0; rb < num_row_blocks_mm1; rb++) {
                        ...
                        for (int cb = 0; cb < num_col_blocks_mm1; cb++) {
                            ...
                            for (int m = 0; m < m_blocks_mm1; m++) {
                                int m_dim = 32; // same for two matrices
                                // write elements to the shared memory for two tiled matrices to be multiplied
                                // ADD here
                                ...
                                for (int sb = 0; sb < num_sub_row_blocks; sb++) {
                                    ...
                                    outputM[blockIdx.x*2*window*negative*hidden_size + rb*16*hidden_size + sb*4*hidden_size + m*32 + 
                                ...
                                for (int sb = 0; sb < num_sub_col_blocks; sb ++) {
                                    ...
                                    inputM[blockIdx.x*batch_size*hidden_size + cb*16*hidden_size + sb*4*hidden_size + m*32 +
                                        ((int)threadIdx.x/32)*hidden_size + (threadIdx.x%32)];
                                }
                                __syncthreads();
                                ...
                                //     int row = r;
                                //     int col = threadIdx.x; // process all columns in parallel (use 16 threads out of 128 threads)
                                //     if (col < col_size) {
                                //         float partial_sum = 0;
                                //         for (int k = 0; k < m_dim; k++) {
                                //             partial_sum += s_vector[row*32+k] * s_vector[16*32 + col*32+k];
                                //         }
                                //         gradM[blockIdx.x*2*window*negative*batch_size + (rb*16+row)*batch_size + (cb*16) + col] += partial_sum;
                                //     }
                                // } // end of r loops
                                __syncthreads();
                            } // end of m loops
                            if (thread_col < col_size) {
                                if (thread_row < row_size)
                                    gradM[blockIdx.x*2*window*negative*batch_size + (rb*16 + thread_row)*batch_size + (cb*16) + thread_col] = reg_tiles[0];
                                if (thread_row + 8 < row_size)
                                    gradM[blockIdx.x*2*window*negative*batch_size + (rb*16 + thread_row + 8)*batch_size + (cb*16) + thread_col] = reg_tiles[1];
                            }
                    ...
                    for (int g = threadIdx.x; g < output_size*batch_size; g += blockDim.x) {
                        float gvalue;
                        float fvalue = gradM[blockIdx.x*2*window*negative*batch_size + g];
                        if (fvalue > MAX_EXP)
                            gvalue = -alpha;
                        ...
                        gradM[blockIdx.x*2*window*negative*batch_size + g] = gvalue;
                    }
                    __syncthreads();
                    ...
                    for (int a = threadIdx.x; a < output_size*hidden_size; a += blockDim.x) {
                        outputMd[blockIdx.x*2*window*negative*hidden_size + a] = 0;
                    }
                    ...
                    for (int rb = 0; rb < num_row_blocks_mm2; rb++) {
                        ...
                        for (int cb = 0; cb < num_col_blocks_mm2; cb++) {
                            ...
                            for (int m = 0; m < m_blocks_mm2; m++) {
                                ...
                                // int num_sub_row_blocks = (32*16+hidden_size-1) / hidden_size;
                                for (int sb = 0; sb < num_sub_row_blocks; sb++) {
                                    s_vector[sb*hidden_size+threadIdx.x] =
                                    gradM[blockIdx.x*2*window*negative*batch_size + rb*32*batch_size + sb*8*batch_size + m*16 +
                                        ((int)threadIdx.x/16)*batch_size + (threadIdx.x%16)];
                                }
                                int num_sub_col_blocks = (col_size*16+hidden_size-1) / hidden_size;
                                // int num_sub_col_blocks = (32*16+hidden_size-1) / hidden_size;
                                for (int sb = 0; sb < num_sub_col_blocks; sb ++) {
                                    s_vector[32*16 + sb*hidden_size+threadIdx.x] = 
                                    inputM[blockIdx.x*batch_size*hidden_size + m*16*hidden_size + sb*4*hidden_size + cb*32 +
                                        ((int)threadIdx.x/32)*hidden_size + (threadIdx.x%32)];
                                }
                                __syncthreads();
                                ...
                                //         float partial_sum = 0;
                                //         for (int k = 0; k < m_dim; k++) {
                                //             partial_sum += s_vector[row*16+k] * s_vector[32*16 + k*32+col];
                                //         }
                                //         outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32+row)*hidden_size + (cb*32) + col] += partial_sum;
                                //     }
                                // } // end of r loops
                                __syncthreads();
                            } // end of m loops
                            ...
                            if (thread_row < row_size)
                                outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32 + thread_row)*hidden_size + (cb*32) + thread_col] = reg_tiles[0];
                            if (thread_row + 4 < row_size)
                                outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32 + thread_row + 4)*hidden_size + (cb*32) + thread_col] = reg_tiles[1];
                            if (thread_row + 8 < row_size)
                                outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32 + thread_row + 8)*hidden_size + (cb*32) + thread_col] = reg_tiles[2];
                            if (thread_row + 12 < row_size)
                                outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32 + thread_row + 12)*hidden_size + (cb*32) + thread_col] = reg_tiles[3];
                            if (thread_row + 16 < row_size)
                                outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32 + thread_row + 16)*hidden_size + (cb*32) + thread_col] = reg_tiles[4];
                            if (thread_row + 20 < row_size)
                                outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32 + thread_row + 20)*hidden_size + (cb*32) + thread_col] = reg_tiles[5];
                            if (thread_row + 24 < row_size)
                                outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32 + thread_row + 24)*hidden_size + (cb*32) + thread_col] = reg_tiles[6];
                            if (thread_row + 28 < row_size)
                                outputMd[blockIdx.x*2*window*negative*hidden_size + (rb*32 + thread_row + 28)*hidden_size + (cb*32) + thread_col] = reg_tiles[7];
                    ...
                    for (int a = threadIdx.x; a < input_size*hidden_size; a += blockDim.x) {
                        inputMd[blockIdx.x*batch_size*hidden_size + a] = 0;
                    }
                        ...
                        for (int cb = 0; cb < num_col_blocks_mm3; cb++) {
                            ...
                            for (int m = 0; m < m_blocks_mm3; m++) {
                                ...
                                if (threadIdx.x < 80) {
                                    ...
                                    // int num_sub_row_blocks = (8*20+80-1) / 80;
                                    for (int sb = 0; sb < num_sub_row_blocks; sb++) {
                                        s_vector[sb*80+threadIdx.x] = 
                                        gradM[blockIdx.x*2*window*negative*batch_size + m*20*batch_size + sb*10*batch_size + rb*8 + 
                                            ((int)threadIdx.x/8)*batch_size + (threadIdx.x%8)];
                                    }
                                }
                                int num_sub_col_blocks = (col_size*20+hidden_size-1) / hidden_size;
                                // int num_sub_col_blocks = (32*20+hidden_size-1) / hidden_size;
                                for (int sb = 0; sb < num_sub_col_blocks; sb++) {
                                    s_vector[8*20 + sb*hidden_size+threadIdx.x] = 
                                    outputM[blockIdx.x*2*window*negative*hidden_size + m*20*hidden_size + sb*4*hidden_size + cb*32 + 
                                        ((int)threadIdx.x/32)*hidden_size + (threadIdx.x%32)];
                                }
                                __syncthreads();
                                ...
                                //         float partial_sum = 0;
                                //         for (int k = 0; k < m_dim; k++) {
                                //             partial_sum += s_vector[k*8+row] * s_vector[8*20 + k*32+col];
                                //         }
                                //         inputMd[blockIdx.x*batch_size*hidden_size + (rb*8+row)*hidden_size + (cb*32) + col] += partial_sum;
                                //     }
                                // }
                                __syncthreads();
                            }
                            ...
                            if (thread_row < row_size)
                                inputMd[blockIdx.x*batch_size*hidden_size + (rb*8 + thread_row)*hidden_size + (cb*32) + thread_col] = reg_tiles[0];
                            if (thread_row + 4 < row_size)
                                inputMd[blockIdx.x*batch_size*hidden_size + (rb*8 + thread_row + 4)*hidden_size + (cb*32) + thread_col] = reg_tiles[1];
                            if (thread_row + 8 < row_size)
                                inputMd[blockIdx.x*batch_size*hidden_size + (rb*8 + thread_row + 8)*hidden_size + (cb*32) + thread_col] = reg_tiles[2];
                            if (thread_row + 12 < row_size)
                                inputMd[blockIdx.x*batch_size*hidden_size + (rb*8 + thread_row + 12)*hidden_size + (cb*32) + thread_col] = reg_tiles[3];
                            if (thread_row + 16 < row_size)
                                inputMd[blockIdx.x*batch_size*hidden_size + (rb*8 + thread_row + 16)*hidden_size + (cb*32) + thread_col] = reg_tiles[4];
                            if (thread_row + 20 < row_size)
                                inputMd[blockIdx.x*batch_size*hidden_size + (rb*8 + thread_row + 20)*hidden_size + (cb*32) + thread_col] = reg_tiles[5];
                            if (thread_row + 24 < row_size)
                                inputMd[blockIdx.x*batch_size*hidden_size + (rb*8 + thread_row + 24)*hidden_size + (cb*32) + thread_col] = reg_tiles[6];
                            if (thread_row + 28 < row_size)
                                inputMd[blockIdx.x*batch_size*hidden_size + (rb*8 + thread_row + 28)*hidden_size + (cb*32) + thread_col] = reg_tiles[7];
                        }
                    }
                    __syncthreads();
                    ...
                    for (int ng_pos = 0; ng_pos < output_size; ng_pos++) {
                        ...
                        // d_Woh[d4 + threadIdx.x] += outputMd[blockIdx.x*2*window*negative*hidden_size + s4 + threadIdx.x];
                        atomicAdd(&d_Woh[d4 + threadIdx.x], outputMd[blockIdx.x*2*window*negative*hidden_size + s4 + threadIdx.x]);
                    }
                    __syncthreads();
                    for (int sen_pos = batch_min_idx; sen_pos < batch_max_idx; sen_pos++) {
                        ...
                        // d_Wih[d3 + threadIdx.x] += inputMd[blockIdx.x*batch_size*hidden_size + s3 + threadIdx.x];
                        atomicAdd(&d_Wih[d3 + threadIdx.x], inputMd[blockIdx.x*batch_size*hidden_size + s3 + threadIdx.x]);
                    }
                    __syncthreads();
                }
            } // endif (sent_length > 0)
...
void dpKernel(int *d_corpus, int *d_sen_len_ptr, float *d_inputM, float *d_inputMd, float *d_outputM, float *d_outputMd, float *d_gradM, int *d_rand_window_batch, int *d_shared_negative, float alpha, int total_number_sentences)
{

    int bDim = hidden_size;
    int gDim = GAMMA * NUM_SMS;
    // printf ("gDim: %d, bDim: %d\n", gDim, bDim);

    
    // For text8, 1b and gigawords
    int total_num_sent = total_number_sentences;
    int num_sentences_pb = (total_num_sent+gDim-1) / gDim;
    // printf("num_sentences_pb = %d\n",num_sentences_pb);

    if (gDim < total_number_sentences) {
        par_sgns_kernel<<<gDim, bDim>>>(num_sentences_pb, window, hidden_size, negative, alpha, table_size, vocab_size, batch_size, total_num_sent, d_table, d_expTable, d_corpus, d_sen_len_ptr, d_Wih, d_Woh, d_inputM, d_inputMd, d_outputM, d_outputMd, d_gradM, d_rand_window_batch, d_shared_negative);
    }
    else {
        // printf("total_num_sent = %d\n",total_num_sent);
        par_sgns_kernel<<<total_num_sent, bDim>>>(num_sentences_pb, window, hidden_size, negative, alpha, table_size, vocab_size, batch_size, total_num_sent, d_table, d_expTable, d_corpus, d_sen_len_ptr, d_Wih, d_Woh, d_inputM, d_inputMd, d_outputM, d_outputMd, d_gradM, d_rand_window_batch, d_shared_negative);
    }
}

void PAR_Word2Vec_GPU() {
...
/**************Load entire dataset and wrtie it to corpus_local[]***************/

    printf("start writing sentences to corpus_local[]\n");
    // load train_file and write to corpus_local

    FILE *fin = fopen(train_file, "rb");
    fseek(fin, 0, SEEK_SET);
    // unsigned long long local_train_words = train_words;
...
/**************wrtie subset of dataset to corpus[]*************************/

    // Allocate HOST memory
    int *corpus;
    corpus = (int *)malloc((train_words+10) * sizeof(int));
    // corpus = (int *)malloc(2501260000 * sizeof(int)); // for gigawords
    int *sen_len_ptr;
    sen_len_ptr = (int *)malloc((MAX_NUM_SENTENCES+1) * sizeof(int));

    printf("start writing sentences to corpus[]\n");
    ...
    while (1) {
        ...
        if (sentence_length == 0) {
            ...
            sentence_position = 0;
        } // up to here all words in each sentence is stored in sen[]

        if ((word_count > train_words)) {
        // if ((disk && feof(fin)) || (word_count > updated_train_words)) {
            break;
        }
    ...
    // #define MAX_NUM_SENTENCES 2 2800 0000  // gigaword

/***********************MATCHED!!!!!!!!!!!!*****************************************/

    printf("word_position = %lld\n",word_position);
    printf("total_word_tokens = %lld\n",total_word_tokens);
    printf("word_count = %lld\n",word_count);
    // printf("updated_train_words = %lld\n",updated_train_words);
    printf("total_number_sentences = %lld\n",total_number_sentences);

    // word_position = 439078761
    // total_word_tokens = 439078761
    // word_count = 804269959
    // updated_train_words = 804269958

/***********************************************************************************/

    // Allocate DEVICE memory
    // Memcpy host to device memory
    ...
    // int GPU_num_blocks = GAMMA * NUM_SMS;

    int *d_corpus, *d_sen_len_ptr;
    checkCUDAerr(cudaMalloc((void **)&d_corpus, word_position * sizeof(int))); // 2501259277
    // printf("end of allocation of d_corpus\n");
    checkCUDAerr(cudaMemcpy(d_corpus, corpus, word_position * sizeof(int), cudaMemcpyHostToDevice));
    checkCUDAerr(cudaMalloc((void **)&d_sen_len_ptr, (total_number_sentences + 1) * sizeof(int))); // 227910245
    // printf("end of allocation of d_sen_len_ptr\n");
    checkCUDAerr(cudaMemcpy(d_sen_len_ptr, sen_len_ptr, (total_number_sentences + 1) * sizeof(int), cudaMemcpyHostToDevice));

    float *d_inputM, *d_inputMd, *d_outputM, *d_outputMd, *d_gradM;
    // checkCUDAerr(cudaMalloc((void **)&d_inputM, (batch_size * hidden_size * GPU_num_blocks) * sizeof(float)));
    checkCUDAerr(cudaMalloc((void **)&d_inputM, (16 * 3 * hidden_size * GPU_num_blocks) * sizeof(float)));
    checkCUDAerr(cudaMalloc((void **)&d_inputMd, (batch_size * hidden_size * GPU_num_blocks) * sizeof(float)));

    // checkCUDAerr(cudaMalloc((void **)&d_outputM, (2 * window * negative * hidden_size * GPU_num_blocks) * sizeof(float)));
    checkCUDAerr(cudaMalloc((void **)&d_outputM, (32 * 3 * hidden_size * GPU_num_blocks) * sizeof(float))); // for text
    // checkCUDAerr(cudaMalloc((void **)&d_outputM, (32 * 4 * hidden_size * GPU_num_blocks) * sizeof(float))); // for graph

    // checkCUDAerr(cudaMalloc((void **)&d_outputMd, (2 * window * negative * hidden_size * GPU_num_blocks) * sizeof(float)));
    checkCUDAerr(cudaMalloc((void **)&d_outputMd, (32 * 3 * hidden_size * GPU_num_blocks) * sizeof(float))); // for text
    // checkCUDAerr(cudaMalloc((void **)&d_outputMd, (32 * 4 * hidden_size * GPU_num_blocks) * sizeof(float))); // for graph

    // checkCUDAerr(cudaMalloc((void **)&d_gradM, (2 * window * negative * batch_size * GPU_num_blocks) * sizeof(float)));
    checkCUDAerr(cudaMalloc((void **)&d_gradM, (32 * 3 * 16 * 3 * GPU_num_blocks) * sizeof(float))); // for text
    // checkCUDAerr(cudaMalloc((void **)&d_gradM, (32 * 4 * 16 * 3 * GPU_num_blocks) * sizeof(float))); // for graph

    // checkCUDAerr(cudaMemset(d_inputM, 0, sizeof(float) * (batch_size * hidden_size * GPU_num_blocks)));

    checkCUDAerr(cudaMemset(d_inputM, 0, sizeof(float) * (16 * 3 * hidden_size * GPU_num_blocks)));
    checkCUDAerr(cudaMemset(d_inputMd, 0, sizeof(float) * (batch_size * hidden_size * GPU_num_blocks)));

    // checkCUDAerr(cudaMemset(d_outputM, 0, sizeof(float) * (2 * window * negative * hidden_size * GPU_num_blocks)));
    checkCUDAerr(cudaMemset(d_outputM, 0, sizeof(float) * (32 * 3 * hidden_size * GPU_num_blocks))); // for text
    // checkCUDAerr(cudaMemset(d_outputM, 0, sizeof(float) * (32 * 4 * hidden_size * GPU_num_blocks))); // for graph

    // checkCUDAerr(cudaMemset(d_outputMd, 0, sizeof(float) * (2 * window * negative * hidden_size * GPU_num_blocks)));
    checkCUDAerr(cudaMemset(d_outputMd, 0, sizeof(float) * (32 * 3 * hidden_size * GPU_num_blocks))); // for text
    // checkCUDAerr(cudaMemset(d_outputMd, 0, sizeof(float) * (32 * 4 * hidden_size * GPU_num_blocks))); // for graph

    // checkCUDAerr(cudaMemset(d_gradM, 0, sizeof(float) * (2 * window * negative * batch_size * GPU_num_blocks)));
    checkCUDAerr(cudaMemset(d_gradM, 0, sizeof(float) * (32 * 3 * 16 * 3 * GPU_num_blocks))); // for text
    // checkCUDAerr(cudaMemset(d_gradM, 0, sizeof(float) * (32 * 4 * 16 * 3 * GPU_num_blocks))); // for graph

    int *d_rand_window_batch, *d_shared_negative;
    checkCUDAerr(cudaMalloc((void **)&d_rand_window_batch, GPU_num_blocks * sizeof(int)));
    // printf("end of allocation of d_rand_window_batch\n");
    checkCUDAerr(cudaMalloc((void **)&d_shared_negative, (2 * window * negative * GPU_num_blocks) * sizeof(int)));
    // printf("end of allocation of d_shared_negative\n");
...
/***********************************************************************************/
    ...
    // float annealing_rate = alpha / iter + (alpha / iter / iter);
    ...
    while (1) {
        ...
        // dpKernel(d_corpus, d_sen_len_ptr, d_inputM, d_inputMd, d_outputM, d_outputMd, d_gradM, d_rand_window_batch, d_shared_negative, alpha, total_number_sentences);
        ...
        // checkCUDAerr(cudaMemcpy(Wih, d_Wih, vocab_size * hidden_size * sizeof(float), cudaMemcpyDeviceToHost));
        ...
        //             fwrite(&Wih[a * hidden_size + b], sizeof(float), 1, foutput);
        //     else
        //         for (int b = 0; b < hidden_size; b++)
        //             fprintf(foutput, "%f ", Wih[a * hidden_size + b]);
        //     fprintf(foutput, "\n");
        // }
        // fclose(foutput);
...
void saveModel() {
    ...
    for (int a = 0; a < vocab_size; a++) {
        ...
        if (binary)
            for (int b = 0; b < hidden_size; b++)
                fwrite(&Wih[a * hidden_size + b], sizeof(float), 1, fo);
        else
            for (int b = 0; b < hidden_size; b++)
