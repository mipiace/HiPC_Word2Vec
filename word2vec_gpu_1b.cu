...
// FIXED parameter!! DONT CHANGE!!!!
// #define MAX_SENTENCE_LENGTH 1000 // text8
// #define MAX_SENTENCE 9300 // text8
// #define MAX_WORDS 1000 // text8

// FIXED parameter!! DONT CHANGE!!!!
#define MAX_SENTENCE_LENGTH 1000 // 1b
#define MAX_SENTENCE 30600000 // 1b
#define MAX_WORDS 20 // 1b

/*#define MAX_SENTENCE_LENGTH 1000 // gigawords
#define MAX_SENTENCE 200000000 // gigawords*/
// #define MAX_SENTENCE 224500000 // gigawords
// #define MAX_WORDS 20

// FIXED parameter!! DONT CHANGE!!!!
/*#define MAX_SENTENCE_LENGTH 1000 // blog
#define MAX_SENTENCE 103120 // blog
#define MAX_WORDS 85 // blog*/

// FIXED parameter!! DONT CHANGE!!!!
/*#define MAX_SENTENCE_LENGTH 1000 // ppi
#define MAX_SENTENCE  38900// ppi
#define MAX_WORDS 85 // ppi*/

// FIXED parameter!! DONT CHANGE!!!!
/*#define MAX_SENTENCE_LENGTH 1000 // wikilabel
#define MAX_SENTENCE 47770 // wikilabel
#define MAX_WORDS 85 // wikilabel*/

// FIXED parameter!! DONT CHANGE!!!!
// #define MAX_SENTENCE_LENGTH 1000 // facebook
// #define MAX_SENTENCE 40300 // facebook
// #define MAX_WORDS 85 // facebook

// FIXED parameter!! DONT CHANGE!!!!
/*#define MAX_SENTENCE_LENGTH 1000 // astro
#define MAX_SENTENCE 187500 // astro
#define MAX_WORDS 85 // astro*/

// FIXED parameter!! DONT CHANGE!!!!
/*#define MAX_SENTENCE_LENGTH 1000 // wiki-large
#define MAX_SENTENCE 4966000 // wiki-large
#define MAX_WORDS 85 // wiki-large*/

// FIXED parameter!! DONT CHANGE!!!!
/*#define MAX_SENTENCE_LENGTH 1000 // umls
#define MAX_SENTENCE 32784000 // umls
#define MAX_WORDS 85 // umls*/
...
void TrainModelThread()
{
  ...
  sen = (int *)malloc(MAX_SENTENCE * MAX_WORDS * sizeof(int));
  sentence_length = (int *)malloc((MAX_SENTENCE + 1) * sizeof(int));

  checkCUDAerr(cudaMalloc((void **)&d_sen, MAX_SENTENCE * MAX_WORDS * sizeof(int)));
  ...
  while (1) {
    ...
    if (feof(fi) || (word_count > train_words)) {                   // Initialize for iteration
      ...
      /*if (local_iter == 9) {
        ...
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_1.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
            if (binary)
                for (int b = 0; b < layer1_size; b++)
                    fwrite(&syn0[a * layer1_size + b], sizeof(float), 1, foutput);
            else
                for (int b = 0; b < layer1_size; b++)
                    fprintf(foutput, "%lf ", syn0[a * layer1_size + b]);
            fprintf(foutput, "\n");
        }
        fclose(foutput);
      }
      ...
      if (local_iter == 8) {
        checkCUDAerr(cudaMemcpy(syn0, d_syn0, vocab_size * layer1_size * sizeof(float), cudaMemcpyDeviceToHost));
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_2.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
      ...
      if (local_iter == 7) {
        checkCUDAerr(cudaMemcpy(syn0, d_syn0, vocab_size * layer1_size * sizeof(float), cudaMemcpyDeviceToHost));
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_3.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
      ...
      if (local_iter == 6) {
        checkCUDAerr(cudaMemcpy(syn0, d_syn0, vocab_size * layer1_size * sizeof(float), cudaMemcpyDeviceToHost));
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_4.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
      ...
      if (local_iter == 5) {
        checkCUDAerr(cudaMemcpy(syn0, d_syn0, vocab_size * layer1_size * sizeof(float), cudaMemcpyDeviceToHost));
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_5.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
      ...
      if (local_iter == 4) {
        checkCUDAerr(cudaMemcpy(syn0, d_syn0, vocab_size * layer1_size * sizeof(float), cudaMemcpyDeviceToHost));
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_6.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
      ...
      if (local_iter == 3) {
        checkCUDAerr(cudaMemcpy(syn0, d_syn0, vocab_size * layer1_size * sizeof(float), cudaMemcpyDeviceToHost));
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_7.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
      ...
      if (local_iter == 2) {
        checkCUDAerr(cudaMemcpy(syn0, d_syn0, vocab_size * layer1_size * sizeof(float), cudaMemcpyDeviceToHost));
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_8.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
      ...
      if (local_iter == 1) {
        checkCUDAerr(cudaMemcpy(syn0, d_syn0, vocab_size * layer1_size * sizeof(float), cudaMemcpyDeviceToHost));
        // const char* default_file = "word2vec_gpu_vectors_epoch_";
        const char* default_file = "word2vec_gpu_vectors_epoch_9.txt";
        FILE *foutput = fopen(default_file, "wb");
        fprintf(foutput, "%lld %lld\n", vocab_size, layer1_size);
        for (int a = 0; a < vocab_size; a++) {
            fprintf(foutput, "%s ", vocab[a].word);
  ...
  FILE *fpOut = fopen("accSGNS_gpu_time", "w");
