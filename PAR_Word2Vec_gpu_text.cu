...
/************** Pre-processing dataset and write it into the corpus ********************/
    ...
    checkCUDAerr(cudaMalloc((void **)&d_M_out, (32 * 3 * hidden_size * GPU_num_blocks) * sizeof(float))); // for text
    checkCUDAerr(cudaMalloc((void **)&d_M_out_update, (32 * 3 * hidden_size * GPU_num_blocks) * sizeof(float))); // for text

    checkCUDAerr(cudaMalloc((void **)&d_M_grad, (32 * 3 * 16 * 3 * GPU_num_blocks) * sizeof(float))); // for text
    ...
    checkCUDAerr(cudaMemset(d_M_out, 0, sizeof(float) * (32 * 3 * hidden_size * GPU_num_blocks))); // for text
    checkCUDAerr(cudaMemset(d_M_out_update, 0, sizeof(float) * (32 * 3 * hidden_size * GPU_num_blocks))); // for text

    checkCUDAerr(cudaMemset(d_M_grad, 0, sizeof(float) * (32 * 3 * 16 * 3 * GPU_num_blocks))); // for text
